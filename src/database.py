...
class Table:
    ...
    def set_value(self, record_name: str, record_id: int, column: str, value):
        sql = f"UPDATE `{self.name}` " f"SET `{column}` = %s WHERE `{record_name}` = %s"
    ...
    def get_value(self, record_name: str, record_id: int, column: str):
        sql = f"SELECT `{column}`" f"FROM `{self.name}` WHERE `{record_name}` = %s"
    ...
    def delete_last_n(self, n_rows: int):
        """
        Delete the last n_rows from the table,
